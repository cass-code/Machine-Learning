draw_confusion_stats <- function(cm) {
    ...
    b <-  arrangeGrob(cm_cl_p1, nrow = 1, ncol = 1, top=textGrob("", gp=gpar(fontsize=14,font=1)))
    c <-  arrangeGrob(cm_cl_p2, nrow = 1, ncol = 1, top=textGrob("", gp=gpar(fontsize=14,font=1)))

    g <- grid.arrange(b, c, nrow = 3, ncol = 1 )
    g
